class CallManager {
    ...
    getReceiverIds(userId) {
        // Assuming you have a method to retrieve receiver IDs for a given user ID
        // For example, if receiver IDs are stored in the user object, you might do:
        const user = this.userStore.get(userId);
        if (user && user.receiverIds) {
            return user.receiverIds;
        } else {
            return []; // Return an empty array if receiver IDs are not found
        }
    }
    ...
    joinCall(userId) {
        ...
        for (let callId in this.calls) {
          ...
          if (call.participants.includes(userId)) {
            console.log("User is already in a call:", callId, call);
            return { success: false, message: 'User is already in a call' };
          }
        }
    
        // Find a suitable call to join
        for (let callId in this.calls) {
          let call = this.calls[callId];
          console.log("Checking call:", callId, call);
          if (!call.participants.includes(userId) && call.participants.length < 2) {
